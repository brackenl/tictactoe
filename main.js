...
let displayController = (() => {
  ...
  function render() {
    for (let i = 0; i < gameBoard.length; i++) {
      if (gameBoard[i]) {
        gridContainer.insertAdjacentHTML(
          "beforeend",
          `<div class="grid-child" id=${i}>${gameBoard[i]}</div>`
        );
      } else {
        gridContainer.insertAdjacentHTML(
          "beforeend",
          `<div class="grid-child" id=${i}></div>`
        );
      }
    }
    ...
    statsContainer.insertAdjacentHTML(
      "beforeend",
      `<div>${crosses.name}: ${crosses.wins} wins</div>`
    );
    statsContainer.insertAdjacentHTML(
      "beforeend",
      `<div>${noughts.name}: ${noughts.wins} wins</div>`
    );
  }
